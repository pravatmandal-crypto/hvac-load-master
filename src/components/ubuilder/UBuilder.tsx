...
const CATEGORY_COLORS: Record<WallCategory, string> = {
  above_ground: 'bg-blue-100 text-blue-800',
  below_ground: 'bg-amber-100 text-amber-800',
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              {assembly.ashraeReference && (
                <span className="text-xs text-gray-400">{assembly.ashraeReference}</span>
              )}
            </div>
            <h3 className="font-semibold text-gray-900 text-sm leading-tight">{assembly.name}</h3>
            {assembly.description && (
              <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{assembly.description}</p>
            )}
          </div>

          <div className="flex flex-col items-end gap-2 shrink-0">
            <div className="text-right">
              <div className="text-2xl font-bold text-blue-700">{assembly.uValue.toFixed(2)}</div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-50">
              {assembly.layers.map((layer, i) => (
                <tr key={i}>
                  <td className="py-1 text-gray-700">{layer.materialName}</td>
                  <td className="py-1 text-right text-gray-600">{layer.thickness} mm</td>
                  <td className="py-1 text-right text-gray-600">{layer.lambda.toFixed(3)}</td>
                  <td className="py-1 text-right font-mono text-gray-700">{layer.r.toFixed(3)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Element Type *</label>
              <select
                value={wallCategory}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-gray-800">
                Layers <span className="text-xs font-normal text-gray-400">(outside → inside)</span>
              </h3>
              ...
              {layers.map((layer, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 p-2">
                  <div className="flex flex-col gap-0.5 text-gray-300">
                    <button onClick={() => moveLayer(i, -1)} disabled={i === 0} className="hover:text-gray-600 disabled:opacity-30">
                      <ChevronUp size={12} />
                    </button>
                    <GripVertical size={12} />
                    <button onClick={() => moveLayer(i, 1)} disabled={i === layers.length - 1} className="hover:text-gray-600 disabled:opacity-30">
                      <ChevronDown size={12} />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-5">
                      <select
                        ...
                        className="w-full rounded border border-gray-200 px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white"
                      >
                    ...
                    <div className="col-span-3 flex items-center gap-1">
                      <input
                        ...
                        className="w-full rounded border border-gray-200 px-2 py-1.5 text-xs text-right focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white"
                      />
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-red-50 hover:text-red-500 text-gray-300 disabled:opacity-20 transition-colors"
                  >
                    <X size={13} />
                  </button>
                </div>
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="col-span-2 sm:col-span-3">
                  <label className="block text-xs font-medium text-amber-800 mb-1">Soil Type</label>
                  <select
                    value={groundParams.soilId}
                    onChange={e => updateGround('soilId', e.target.value)}
                    className="w-full rounded border border-amber-200 bg-white px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400"
                  >
                    {SOIL_TYPES.map(s => (
                ...
                <div>
                  <label className="block text-xs font-medium text-amber-800 mb-1">Depth — Top (m)</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-medium text-amber-800 mb-1">Depth — Bottom (m)</label>
                  <input
                ...
                <div>
                  <label className="block text-xs font-medium text-amber-800 mb-1">Slope Angle (°)</label>
                  <input
                    ...
                    className="w-full rounded border border-amber-200 bg-white px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400"
                  />
                  <p className="text-[10px] text-amber-600 mt-0.5">0° = vertical backfill face</p>
                </div>
              </div>
            </div>
          )}
          ...
          {result ? (
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-5 py-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-blue-500 font-medium uppercase tracking-wide">Calculated U-Value</p>
                  <p className="text-4xl font-bold text-blue-700 mt-0.5">
                    {result.uValue.toFixed(3)}
                    <span className="text-base font-normal text-blue-400 ml-1">W/m²K</span>
                  </p>
                </div>
                <div className="text-right text-xs text-blue-600 space-y-0.5">
                  ...
                  <div className="text-[10px] text-blue-400">{result.method}</div>
                </div>
              </div>
              ...
              {result.strips && result.strips.length > 1 && (
                <details className="mt-3">
                  ...
                  <div className="mt-2 max-h-36 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="text-blue-400 text-[10px]">
                      ...
                      <tbody>
                        {result.strips.map((s, i) => (
                          <tr key={i} className="border-t border-blue-100">
                            <td className="py-0.5 text-blue-700">{s.depthMidpoint.toFixed(2)}</td>
                            <td className="text-right text-blue-600">{s.stripHeight.toFixed(2)}</td>
                            <td className="text-right font-mono text-blue-800">{s.uAtDepth.toFixed(3)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-5 py-4 text-center text-sm text-gray-400">
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">U Builder</h1>
        ...
        <div className="flex-1 flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-200 bg-white pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <select
            value={categoryFilter}
        ...
        <div>
          {loading ? (
            ...
            <div className="rounded-xl border-2 border-dashed border-gray-200 py-16 text-center">
              ...
              <p className="text-gray-500 font-medium">No custom assemblies yet</p>
